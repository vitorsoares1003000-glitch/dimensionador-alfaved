...
# =============================================================================
# 2. CSS CUSTOMIZADO (string limpa, sem quebras estranhas)
# =============================================================================
st.markdown("""
<style>
    .main-header {
...
def calc_reynolds(vazao_kgh, visc, dens, dh, w_placa):
    if visc <= 0 or dens <= 0 or w_placa <= 0:
        return 0
    v_ms = (vazao_kgh / 3600.0) / (dens * w_placa * 0.003)
...
def generate_dimensional_svg(mod_key, n_placas):
    ...
    svg = f"""<svg width="500" height="350" viewBox="0 0 500 350" xmlns="http://www.w3.org/2000/svg">
...
def main():
    st.markdown(
        '<div class="main-header"><h1>AlfaVed Dimensionador PHE</h1>'
        '<p>Engenharia Termica de Alta Performance</p></div>',
        unsafe_allow_html=True
    )
    ...
    with tab2:
        if 'res' in st.session_state:
            ...
            st.markdown(f"### Desenho Tecnico: {mod}")
            st.components.v1.html(generate_dimensional_svg(mod, res['placas']), height=400)

            st.markdown("#### Especificacoes do Frame")
            m = BANCO_MODELOS[mod]
            st.write(f"**Conexao:** {m['conn']} | **Pressao Max:** {m['Pmax']} bar | **Temp Max:** {m['Tmax']} C | **Peso:** {m['weight']} kg")
        else:
